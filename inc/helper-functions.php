...
// Inline style
function fmplayer_style() {
    wp_enqueue_style(
        'fm-custom-style',
        get_template_directory_uri() . '/assets/css/custom.css'
    );
        $fm_bg = arya_multipurpose_get_option( 'arya_multipurpose_fm_bg' ); //E.g. #FF0000
        $custom_css = "
                body.home{
                    background-image: url({$fm_bg});
                }";
        wp_add_inline_style( 'fm-custom-style', $custom_css );
}
add_action( 'wp_enqueue_scripts', 'fmplayer_style' );